...
import { PlatformsOption, GenresOption, RegionsOption} from "@blocks/SearchOption/SearchOption";
...
function DiscoverDialog({isOpen, onClose}:Props) {
    ...
    const onChange = useCallback((type: keyof Filter, target: number|string, active:boolean) => {
        if(active){
            setFilter(prev => ({...prev, [type] : [...prev[type], target]}))
            return;
        }
        setFilter(prev => (
            {...prev, [type]: (prev[type] as Array<string|number>).filter(item => item !== target)}
        ))
    },[]);
    ...
    return ( isOpen ? <Dialog onClose={onClose} Buttons={<Button onClick={onClickDiscover}>영화 찾기 🔍</Button>}>
        <Wrap>
